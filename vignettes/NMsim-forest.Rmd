...
* Specify covariates to simulate using `expandCovs()`
* Simulate with parameter uncertainty by one of
* Based on $COV using `NMsim_NWPRI()` or `NMsim_VarCov()`
...
* Summarize covariate effect estimates with confidence intervals, using `summarizeCovs()`
* Plot the summary as a forest plot

Each step is facilitated with efficient interfaces (R functions) and automated execution. Essentially, each step above is one function call. 
...
```{r}
doses <- NMcreateDoses(TIME=0,AMT=30,ADDL=29,II=24)
doses
```
...
## Specify covariates to analyze
`NMsim::expandCovs()` can be used to construct a set of covariate value combinations needed to derive a forest plot. It varies one covariate at a time, keeping all other covariates at their reference value. It can derive references and quantiles from a data set.
...
covs <- expandCovs(
    ...
    WEIGHTB=list(ref=median, quantiles=c(10,25,75,90)/100, label="Bodyweigt (kg)"), ## Bodyweight by quantiles
...
- We read the estimation data set `data.ref`, and pass that as the `data` argument to `expandCovs()`. Using this, `expandCovs()` can evaluate specified quantiles of covariates on the observed or any desired population.
- We specify reference values and simulation covariate values as specific values (see `AGE`) or  covariate quantiles (`WEIGHTB`). If quantiles are derived by `expandCovs()`, the names of the covariates (here, `AGE`, `WEIGTB`, `MALEN`) must be columns in `data`. 
- For categorical variables, we can label numeric values like `ref=c(Female=0)` which means we label `MALEN==0` as "Female".

When deriving quantiles, `expandCovs()` will first derive unique covariate values for each subject in the data set, then derive the quantiles. Each subject hence contributes equally, independently of how many data points they contribute. 
...
We shall use the method provided
by NMsim based on the multivariate normal distribution
(`mvrnorm`). This method is selected because it is adequate for
sampling `THETA`'s, it does not require additional software installed,
and it is robust.
...
ext.simpar <- sampleParsSimpar(file.mod=file.mod,nsim=1000,seed.R=6789,as.fun="data.table")
## source("~/wdirs/NMdata/R/addBlocks.R")
## ext.simpar <- addBlocks(ext.simpar)[]
...
## simres.forest.simpar <- NMsim(file.mod # path to NONMEM model
##                              ,data=dt.sim.covs, # simulation dataset
##                              ,name.sim="simpar_forest_tmp" # output name suffix
##                              ,method.sim=NMsim_VarCov # sampling with mvrnorm
##                              ,ext=ext.simpar[model<=5]
##                              ,typical=TRUE # FALSE to include BSV
##                              ,table.vars=cc(PRED,IPRED) # output table variables
##                              ,seed.R=342 # seed for reproducibility
...
## dir.bs <- "NMsim-forest-models/xgxr134_bs_N250/modelfit_dir1/"
dir.bs <- "NMsim-forest-models/xgxr134_bs_N1000/modelfit_dir1/"
lsts.bs <- list.files(dir.bs,pattern=".+\\.lst$",recursive=T,full.names = TRUE)
## fun.name <- function(file)sub(".*/NM_run([0-9]+)/psn....","boot\\1",file)
fun.name <- function(file)sub(".*/NM_run([0-9]+)/psn....","\\1",file)  |>
                          as.integer()
## sort(fun.name(lsts.bs))
ext.boot <- NMreadExt(lsts.bs,as.fun="data.table")
ext.boot <- NMreadExt(lsts.bs,modelname=fun.name,as.fun="data.table")
##ext.boot <- NMreadExt(lsts.bs,modelname=identity,as.fun="data.table")
##ext.boot[,model:=fun.name(model)]
...
setnames(sum.uncertain,
         ...
         ##cc(predmm,predml,predmu,metric.var),
         ##cc(mid,lower,upper,paramname)
         )
...
<details>
<summary>Comparison of different simulation methods</summary>
...
as.data.table(simres.forest.all)[,lapply(.SD,uniqueN),.SDcols=c("model","model.sim","NMREP"),by=method]|>
    melt(measure.vars=c("model","model.sim","NMREP"),variable.name="Column",value.name="Unique values")

## load_all("~/wdirs/NMsim")
sum.uncertain <- summarizeCovs(simres.forest.all,
                               funs.exposure = funs.exposure,
                               by=cc(method,period),
                               cover.ci=.95,
                               as.fun="data.table"
...
ggplot(sum.uncertain[period=="Steady-State"],aes(predmm,covvalf,colour=method))+
    geom_rect(aes(xmin=.8,xmax=1.2,ymin=-Inf,ymax=Inf,colour=NULL),alpha=.1)+
    ## geom_segment(aes(x=predml,xend=predmu,y=covvalf,yend=covvalf),
    ...
    labs(y="",x="Relative Covariate Effect")
