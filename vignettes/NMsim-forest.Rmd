...
```{r}
NMdata::NMreadExt(file.mod)[par.type=="THETA",.(RSE=se/value),by=.(par.name,FIX)] |>
    NMdata::mergeCheck(NMrelate(file.mod,par.type="THETA")[,.(par.name,code)],by="par.name",quiet=TRUE)
```
...
The step reuired by the user is to define the functions to derive relevant exposure metrics. We will use AUC 0-24h and Cmax.
...
#forest.day1 <- fun.plot(sum.uncertain[period=="Day 1"],plot_title="Day 1")
forest.ss <- fun.plot(sum.uncertain[period=="Steady-State"],plot_title="Steady-State")
...
We are including the code from the main steps in the post processing function, `NMsim::summarizeCovvar()`. It is important that the scientist understands that what the forest plots derived in this document represent is the relative effect of the covariate on the exposure metric. This is derived by `NMsim::summarizeCovvar()` as quantiles of the exposure relative to the reference subject. The confidence interval hence expresses the uncertainty on the covariate effect for a subject with other covariates at reference values.


Notice, the code below consists of snippets from the
`NMsim::summarizeCovvar()` function. The code is not intended to be
used on its own. 
