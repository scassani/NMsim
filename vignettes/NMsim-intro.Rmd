...
knitr::write_bib(c('rmarkdown','pagedown','data.table','NMdata','NMsim','xgxr'), 'packages.bib')
```
...
<details>
<summary>Simulate 1000 new subjects using `$SUBPROBLEMS`</summary>
...
<details>
<summary>Simulate 1000 new subjects with covariate sampling</summary>
```{r,subproblems,include=TRUE,echo=TRUE,eval=doSims}
...
```{r,multisubjs-show,eval=TRUE,echo=TRUE}
...
ggplot(simres.pi,aes(TIME,fill=trt))+
    ...
    scale_alpha_manual(values=c("90% Prediction interval"=.5))+
    facet_wrap(~name.sim)+
    labs(x="Hours since first dose",y="Concentration (ng/mL)")+
    theme(legend.position="none")
```
