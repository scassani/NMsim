...
This NMsim vignette aims at enabling NONMEM users to modify and run a model directly from within R. This option allows the users to run simulations with parameters modified from theestimated values. The examples described below will address the following pharmacometric questions:
...
* How is the concentration-time profile affected if switching formulation (reducing absorption rate) by a range of fold-values?
* What is the expected concentration-time profile in patients with a certain Drug-Drug Interaction effect on clearance and bioavailability?
* How will a dose delay of different amounts of time affect the predicted exposure?
* How can AUC computation over a coarse time grid be integrated in the simulation via `NMsim`?

<!-- Philip comment: This wording can be clearer. Use nonmem to integrate AUC, with benefit that a sparse simulation grid can be used? -->
...
Sometimes we want to simulate with some modification to the estimated model. NMsim canmake such user-specified modifications to the model before simulating through the `modify.model` argument.
...
sim.alag.AUC<-sim.alag.final[DAY<8,.(AUC=trapz(TIME,PRED))
...
* at post-processing, using trapezoidal method, after the unmodified NONMEM model is simulated on three different fine grids with evenly spaced time steps of `0.25`, `1`, and `4` hours, respectively, labelled __AUC trapz__
...
sres.trapz <- NMsim(file.mod=file.mod.auc
...
sim.auc=sres.trapz[EVID==2,.(ID,TIME,PRED,time.step=TSTEP)] 
...
sim.auc.trapz<-sim.auc.final[DAY<8,.(AUC=pracma::trapz(TIME,PRED))
                             ,by=.(ID,DAY,time.step)]
...
```
The plot comparing the `DES` and `trapz` AUC is obtained with the code below

```{r, echo=T, eval=T,fig.height=3.7,fig.cap="Daily exposures computed at run time ($DES, coarse grid, x-axis) and post-processing time (`trapz`, fine grids, y-axis). AUC (trapz) converges to the value computed with $DES method as the time step is reduced. Includes identity line."}

sres.final=mergeCheck(sim.auc.trapz[,.(DAY,AUC,time.step)]
